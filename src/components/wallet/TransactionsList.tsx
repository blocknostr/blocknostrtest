...
const TransactionsList = ({ address, updateApiStatus, apiHealth }: TransactionsListProps) => {
  ...
  const getTransactionAmount = (tx: Transaction) => {
    ...
    if (type === 'received') {
      ...
      const relevantOutputs = tx.outputs.filter(output => output.address === address);
  ...
  const getCounterpartyAddress = (tx: Transaction) => {
    ...
    if (type === 'received') {
      ...
      // In Alephium, inputs might not directly contain the address, 
      // so we look for the first input with an address or return 'Unknown'
